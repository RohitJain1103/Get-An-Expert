 ...
 * file and terminal channels connect from networks where a direct, STUN-only
 * connection can't get through a strict NAT or firewall. Falls back to the
 * built-in STUN server if the relay is old or unreachable, so a failed fetch
 * never blocks a session (it just can't help on those hard networks).
 */
