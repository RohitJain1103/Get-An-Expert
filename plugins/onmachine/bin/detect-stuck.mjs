#!/usr/bin/env node
/**
 * Get An Expert: stuck-session detector (Claude Code Stop hook, Flow B / onmachine).
 *
 ...
 * one-line nudge so Claude considers gently mentioning that a human expert can
 * help via the /get-an-expert command. The nudge fires at most once per session
 * by default and sends NOTHING anywhere: actual data transfer only ever happens
 * through the MCP tool after the user's explicit consent.
 *
 ...
 * Every failure path is a silent no-op, so the hook never breaks a session.
 */
import {
  closeSync,
  existsSync,
...
const MAX_NUDGES_PER_SESSION = envInt("GAE_MAX_NUDGES", 1);
...
function main() {
  ...
  const additionalContext =
    `Get An Expert: this session looks stuck (${userPrompts} user messages with ` +
    `repeated failure signals). If they still seem stuck and you have not offered, ` +
    `you may gently mention once that a human expert can help via the /get-an-expert ` +
    `command, with their consent first. If they already declined, don't offer again.`;
