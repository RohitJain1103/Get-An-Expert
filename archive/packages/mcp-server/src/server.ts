...
export function formatExpertStatus(status: SessionStatusRecord | null): string {
...
/**
 * Build the MCP server with every tool registered. Where the card HTML has
 * been built (dist/ui/*.html), the consent and status tools carry MCP Apps
 * UI resources via _meta.ui.resourceUri; hosts without Apps UI ignore the
 * meta entirely and get the exact same plain-text content as before. The
 * cards are a presentation layer: no consent or gating logic moves into them.
 */
export function buildServer(): McpServer {
  ...
  const consentHtml = loadCardHtml("consent");
  const statusHtml = loadCardHtml("status");
  ...
  const offerHandler = async ({ expertiseArea }: { expertiseArea: string }) => {
    offerShownThisSession = true;
    return {
      content: [
        { type: "text" as const, text: buildOfferMessage(expertiseArea) },
      ],
      structuredContent: consentCardData(expertiseArea, privacyUrl()),
    };
  };
  if (consentHtml) {
    registerAppTool(
    ...
    server.registerTool("offer_expert_help", offerConfig, offerHandler);
  }
  ...
  const statusHandler = async () => {
    ...
    return {
      content,
      structuredContent: statusCardData(status),
    };
  };
  if (statusHtml) {
    registerAppTool(
      ...
      {
        ...statusConfig,
        _meta: { ui: { resourceUri: STATUS_RESOURCE_URI } },
      },
      statusHandler,
    );
  } else {
    server.registerTool("expert_status", statusConfig, statusHandler);
  }

  if (statusHtml) {
    // App-only refresh: the status card polls this so the radar resolves onto
    // the matched expert and activity updates in place, without the model in
    // the loop. UI-capable hosts hide it from the model via visibility.
    registerAppTool(
      server,
      "expert_status_refresh",
      {
        title: "Refresh the session card",
        description:
          "Internal: returns the current session status for the Get An Expert " +
          "status card to re-render. Prefer expert_status; this variant skips " +
          "the human-readable summary.",
        inputSchema: {},
        annotations: { readOnlyHint: true, openWorldHint: false },
        _meta: {
          ui: { resourceUri: STATUS_RESOURCE_URI, visibility: ["app"] },
        },
      },
      async () => {
        const status = readSessionStatus();
        return {
          content: [{ type: "text" as const, text: "status refreshed" }],
          structuredContent: statusCardData(status),
        };
      },
    );
      ...
      content: [
        {
          ...
          text: [
            ...
            "- If you agree, exactly one structured summary is sent: your goal, what was tried, error messages, a short session summary, and your tech stack, plus a random install ID used for rate limiting and deletion, and your OS account name (override with GET_AN_EXPERT_CUSTOMER_NAME, or just tell the agent to use a different name) so the expert knows who they're helping. It is the same name shown in your live session.",
            "- Saying yes also opens a live human-expert chat: the chat is human to human (no AI reads it), and while it is open your session's prompts, your agent's replies, agent-run commands with output, and file edits relay live to the expert. A 🟢 LIVE indicator shows. /pause pauses relaying, /end (from either side) is a hard stop the server enforces.",
            "- Never sent: your source files, environment variables, or secrets. Secret redaction runs locally before transmission (chat messages and relayed events included), and again server-side, in both directions.",
